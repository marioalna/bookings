class TailwindFormBuilder < ActionView::Helpers::FormBuilder
  ...
  SELECT_CLASSES = "form-input-base px-3 py-2.5 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 block w-full text-sm border-gray-300 rounded-lg"
  CHECKBOX_CLASSES = "form-checkbox focus:ring-indigo-500 h-5 w-5 text-indigo-600 border-gray-300 rounded"
  FILE_CLASSES = "form-input-base block w-full text-sm text-gray-500 file:mr-3 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 border border-gray-300 rounded-lg cursor-pointer"
  ...
  def file_input(name, *args)
    ...
    @template.content_tag(:div, { class: "form-element #{FIELD_SPACING}", id: "field-#{name}" }) do
      @template.concat(label(label_text, { class: LABEL_CLASSES + (error ? " text-red-700" : "") }))
      @template.concat(@template.content_tag(:div, { class: "mt-2" }) do
        @template.concat(send(:file_field, name,
                              { class: FILE_CLASSES + (error ? " #{INPUT_ERROR_CLASSES}" : "") }.merge(options)))
      end)
  ...
  def country_input(name, country_options = {}, html_options = {})
    html_options.merge!(class: "mt-2 #{SELECT_CLASSES}")
        ...
        label("#{name}_#{tag_value}",
          ...
          @template.concat(@template.content_tag(:span, { class: "flex flex-1 pointer-events-none" }) do
                             @template.concat(@template.content_tag(:span, { class: "flex flex-col pointer-events-none" }) do
                                                ...
                                                @template.concat(@template.content_tag(:span,
                                                                                       class: "mt-2 flex items-center text-sm text-gray-500") do
                                                                   description_text
                                                                 end)
                                                @template.concat(@template.content_tag(:span,
  ...
  def radio_input(name, tag_value, *args)
    ...
    @template.content_tag(:div, { class: "form-element #{FIELD_SPACING}", id: "field-#{name}" }) do
      @template.concat(@template.content_tag(:div, { class: "flex items-center" }) do
        @template.concat(radio_button(name, tag_value,
                                      { id:, class: "focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" + (error ? " #{INPUT_ERROR_CLASSES}" : "") }.merge(options)))
        @template.concat(label("#{name}_#{tag_value}", label_text,
                               { class: "ml-3 block #{LABEL_CLASSES} " + label_class + (error ? " text-red-700" : ""), for: id }))
      end)
      @template.concat(hint_message(message, error))
    end
  end
  ...
  def collection_select_input(name, collection, value_method, text_method, options = {}, html_options = {})
    ...
    html_options.merge!(class: "#{SELECT_CLASSES} #{style}" + (error ? " #{INPUT_ERROR_CLASSES}" : ""))
    @template.content_tag(:div, { class: "form-element mt-2", id: "field-#{name}" }) do
  ...
  def collection_time_select(name, options = {}, html_options = {})
    html_options.merge!(class: "shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm border-gray-300 rounded-lg")
    ...
    @template.content_tag(:div, { class: "form-element", id: "field-#{name}" }) do
      @template.concat(@template.content_tag(:div, { class: "flex items-center" }) do
        @template.concat(check_box(name,
                                   { class: CHECKBOX_CLASSES + (error ? " #{INPUT_ERROR_CLASSES}" : "") }.merge(options), "true", "false"))
        @template.concat(label(name, label_text.presence,
                               { class: "ml-3 text-sm font-medium text-gray-700 cursor-pointer" + (error ? " text-red-700" : "") }))
      end)
      @template.concat(hint_message(message, error))
    end
  end
  ...
  def submit_button(name, *args, &)
    args << { class: "inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" }
    button(name, *args, &)
  end
  ...
  private
    ...
    def input_element(field_type, name, *args, &block)
      ...
      @template.content_tag(:div, { class: "form-element #{FIELD_SPACING}", id: "field-#{name}" }) do
        @template.concat(label(label_text,
                               { class: LABEL_CLASSES + (error ? " text-red-700" : "") }))
        @template.concat(@template.content_tag(:div, { class: "relative mt-2 rounded-lg shadow-sm" }) do
          @template.concat(show_icon(block)) if block.present?
          @template.concat(send(field_type, name,
                                { class: "#{INPUT_CLASSES} #{styles}" + (block.present? ? " pl-10" : "") + (error ? " #{INPUT_ERROR_CLASSES}" : "") }.merge(options)))
          @template.concat(error_icon) if error.present?
        end)
        @template.concat(hint_message(message, error))
      end
    end
    ...
    def float_input_element(field_type, name, *args)
      ...
      @template.content_tag(:div, id: "field-#{name}") do
        @template.concat(@template.content_tag(:div, { class: "relative" }) do
          @template.concat(send(field_type, name,
                                { class: "block px-2.5 pb-2.5 pt-6 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" + styles + (error ? " border-red-300 focus:border-red-600" : "") }.merge(options)))
          @template.concat(label(label_text,
                                 { class: "absolute text-sm pt-2 text-gray-500 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1" + (error ? " text-red-600" : "") }))
          @template.concat(hint_message(message, error))
          @template.concat(error_icon) if error.present?
        end)
      end
    ...
    def range_element(name, *args)
      ...
      @template.content_tag(:div, { class: "relative", id: "field-#{name}" }) do
        @template.concat(label(label_text, { class: LABEL_CLASSES }))
        @template.concat(
          range_field(
            name,
            {
              min: "0", max: "10", step: "1",
              class: "w-full h-2 mt-2 bg-blue-100 rounded-lg appearance-none cursor-pointer " + (error ? " #{INPUT_ERROR_CLASSES}" : "")
            }.merge(options)
          )
        )
        @template.concat(hint_message(message, error))
    ...
    def date_or_time_element(field_type, name, *args, &block)
      ...
      @template.content_tag(:div, { class: "form-element #{FIELD_SPACING}", id: "field-#{name}" }) do
        @template.concat(label(label_text, { class: LABEL_CLASSES }))
        @template.concat(@template.content_tag(:div, { class: "relative mt-2" }) do
          @template.concat(show_icon(block)) if block.present?
          @template.concat(send(field_type, name,
                                { class: "#{INPUT_CLASSES}" + (block.present? ? " pl-10" : "") + (error ? " #{INPUT_ERROR_CLASSES}" : "") }.merge(options)))
        end)
        @template.concat(hint_message(message, error))
      end
    end
    ...
    def hint_message(message, error)
      ...
      @template.content_tag(:p, class: error ? HINT_ERROR_CLASSES : HINT_CLASSES) do
        message
      end
    end
